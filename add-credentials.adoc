...
详细了解 link:permissions-reference.html["权限参考"] 中每个工作负载功能所需的权限。
...
. 从菜单中选择“*管理*”，然后选择“*凭据*”。
. 在“凭据”页面上，选择*添加凭据*。
. 在添加凭据页面上，选择 *Add manually*，然后按照以下步骤完成 _Permissions configuration_ 下的每个部分。
+
...
. 对于那些提供权限策略选择的工作负载功能，请选择使用这些凭据可获得的权限类型。
+

NOTE: 如果您有 GovCloud 帐户，则不需要 Storage 的 **View, planning, and analysis** 策略中的 Cost Explorer 权限 `ce:GetCostAndUsage`或 `ce:GetTags`。从代码框中复制脚本后，请删除这些权限，并删除上一行中的尾随逗号。
...
. 复制角色ARN。
. 返回 Workload Factory 中的 Add Credentials 页面，然后将角色 ARN 粘贴到 _Role ARN_ 字段中。
...
NOTE: 使用 CloudFormation 向 AWS GovCloud (US) 账户添加凭据不受支持。
