...
---
= 在 NetApp Workload Factory 中管理服务账户
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. 或者，您可以通过执行客户端凭据交换来获取 Auth0 管理 API 的访问令牌。有关生成刷新令牌的信息，请参阅 link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console API 文档"^]。
+
curl 示例展示了如何获取客户端 ID 和密钥，并使用 API 生成有时间限制的访问令牌。令牌提供对 NetApp Workload Factory API 的几个小时的访问权限。
...
----
+
将输出发布到 \https://netapp-cloud-account.auth0.com/oauth/token 以获取访问令牌。
