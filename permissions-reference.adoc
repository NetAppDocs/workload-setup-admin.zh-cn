...
当您提供权限时，Workload Factory 会将具有权限的策略附加到实例，以管理该 AWS 帐户内的资源和流程。这允许 Workload Factory 执行各种操作，从发现存储环境到在存储管理中部署 AWS 资源（如文件系统）。
...
NOTE: AWS GovCloud 不支持 AWS Cost Explorer，因此您不需要这些权限： `ce:GetCostAndUsage` 和 `ce:GetTags`。如果您将其包含在内，该策略仍然有效。
...
| 检索 AWS CloudTrail 跟踪的当前事件选择器配置 | cloudtrail:GetEventSelectors | S3 访问点日志操作 | 运营和补救 


| 通过启用或禁用 FSx for ONTAP S3 接入点与 CloudTrail 跟踪之间的关联来配置或更新 AWS CloudTrail 事件选择器 | cloudtrail：PutEventSelectors | S3 访问点日志操作 | 运营和补救 
|===
====
...
| 从已注册的 SQL 节点收集已保存到 Amazon CloudWatch 的性能指标。数据将在已注册 SQL 实例的管理实例屏幕上生成性能趋势图。 | CloudWatch：GetMetricData | 清单 | 运营和补救 
...
| 在发送部署后用于管理操作的命令后轮询 SSM 响应 | SSM：GetCommandInvation  a| 
* 管理操作
* 清单
* 了解节省量
* 优化
...
| 获取 Windows EC2 实例上的修补清单详细信息，用于评估和修正工作流程 | ssm:DescribeInstancePatches | 优化 | 视图、规划和分析 


| 获取可用于操作系统补丁评估的补丁列表 | ssm:DescribeAvailablePatches | 优化 | 视图、规划和分析 


| 检索从托管 SQL 节点收集的库存元数据 | ssm:GetInventory | 清单 | 视图、规划和分析 


| 列出从托管 SQL 节点收集的库存条目 | ssm:ListInventoryEntries | 清单 | 视图、规划和分析 
...
| 获取可用于错误日志分析的基础模型 | Bound：GetFoundation论 可用性 | 错误日志分析 | 运营和补救 


| 列出 Amazon Bedrock 中可用于错误日志分析的接口配置文件 | Bedrock：ListInferenceProfile | 错误日志分析 | 运营和补救 
|===
. 权限仅限于从“资源管理模块”开始的资源。
...
通过操作和补救权限，您可以获得全面的架构良好的评估，根据架构良好的分析应用建议的操作，对资源进行更改并分析错误日志。
