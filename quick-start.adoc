...
[role="quick-margin-para"]
此步骤为可选步骤。您可以使用 Workload Factory 而无需添加凭据来访问您的 AWS 账户。向 Workload Factory 添加 AWS 凭据将为您的 Workload Factory 账户提供创建和管理 FSx for ONTAP 文件系统以及部署和管理数据库等特定工作负载所需的权限。
