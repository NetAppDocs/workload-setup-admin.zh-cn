...
Lambda链路连接:: 使用 Lambda 链接连接在 Workload Factory 帐户与一个或多个 FSx for ONTAP 文件系统之间创建信任关系和连接。链接让您可以直接通过 ONTAP REST API 调用来监控和管理某些文件系统功能，而这些功能无法通过 Amazon FSx for ONTAP API 获得。
...
EDA存储操作:: 通过优化性能、与 IaC 集成以自动化存储参数并确保操作健康，简化 NetApp Workload Factory 中的电子设计自动化 (EDA) 存储操作。
...
NOTE: 账号管理员可以开启或关闭 AI 功能。如果管理员将其关闭，则 Ask Me AI 助手将不可用。link:manage-genai-features.html["了解如何管理 AI 功能"]。
...
NetApp Workload Factory的安全性是NetApp的首要任务。Workload Factory 中的所有工作负载均运行在Amazon FSx for NetApp ONTAP之上。除此之外，还有所有 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/security.html["AWS 安全功能"^]NetApp Workload Factory已收到 link:https://trust.netapp.com/["SOC2 Type 1 合规性、SOC2 Type 2 合规性和 HIPAA 合规性"^]。
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/links-overview.html["详细了解链接"^]
...
Workload Factory 可帮助您维护和运行符合 AWS 良好架构框架的可靠、安全、高效且经济的存储和数据库配置。Workload Factory 每天扫描 FSx 中的ONTAP文件系统、SQL Server 和 Oracle 数据库部署情况，以提供有关潜在错误配置的见解，并建议手动或自动操作来修复问题。

link:well-architected-workloads.html["了解更多关于架构良好的工作负载的信息"]
...
* *NetApp Terraform 提供程序*：使用 Terraform 构建和管理在 Workload Factory 控制台中生成的基础设施工作流。
